class Company{
    ...
    bestDepartment(){
        let department = this.departments.sort((a, b) => b.employees.reduce((x, y) => x + y) - a.employees.reduce((x, y) =>x + y))[0];
   
        return `Best Department is: ${department.department}\nAverage salary: ${(department.employees.map(x => x.salary)
                                                       .reduce((x, y) => x + y) / department.employees.length)
                                                       .toFixed(2)}\n${department.employees.sort((a, b) => b.salary - a.salary || a.username.localeCompare(b.username))
                                                       .map(x => `${x.username} ${x.salary} ${x.position}`).join("\n")}`;
    }
}
